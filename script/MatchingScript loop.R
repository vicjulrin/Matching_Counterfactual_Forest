...
with_progress(result <- {
  p <- progressor(along= seq_along(gov_list))
  
  result<-future_lapply( gov_list, function(type_gov) {p();
    ...
    tryCatch({
      ...
      summary_forest<- data.frame(fd= c("treatment", "control_pre", "control_pos"),
                                  ...
                                  zval_M_forest_covs = c(sign_Model_M_forest_covs_sum, NA, NA)  ) %>% 
        ...
        dplyr::mutate(sign_forest_covs= sapply(.$zval_M_forest_covs, function(x) {
          if(is.na(x)){""}else if(x<0.001){"***"}else if(x<0.05){"**"}else if(x<0.1){"*"}  else {""} } )  )
      
      
      
      write.xlsx(summary_forest, file.path(dir_work, paste0(type_gov,  "_summary_forest.xlsx")) )
